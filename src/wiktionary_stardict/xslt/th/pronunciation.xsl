...
<xsl:stylesheet
  ...
  <xsl:template match="tr[normalize-space(th[1]) = 'ไฟล์เสียง']" mode="th-pron-tr"/>
  <xsl:template match="th/div[normalize-space(.) = 'แก้ไข']" mode="th-pron-tr"/>
  <xsl:template match="td/sup[normalize-space(.) = '(สัมผัส)']" mode="th-pron-tr"/>
</xsl:stylesheet>
